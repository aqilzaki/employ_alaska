...
// ... (DEPARTMENTS_CONFIG_RAW tetap sama) ...
const DEPARTMENTS_CONFIG_RAW = {
  'operator':       { count: 0, color: 'from-gray-400 to-gray-600' },
  'marketing':      { count: 0, color: 'from-orange-400 to-orange-600' },
  'it':             { count: 0, color: 'from-purple-400 to-purple-600' },
  'hrd':            { count: 0, color: 'from-pink-400 to-pink-600' },
  'direktur':       { count: 0, color: 'from-blue-400 to-blue-600' },
};
...
export default function EmployeeDashboard() {
  ...
  const loadData = async () => {
    try {
      const [employeeData, jabatanData, statusData] = await Promise.all([
        getEmployees(), 
        getJabatanOptions(),
        getStatusKerjaOptions()
      ]);
  ...
  useEffect(() => {
    ...
    if (path.includes('/karyawan')) {
      setActiveMenu('karyawan');
    } else {
      setActiveMenu('dashboard');
    }
    setIsSidebarOpen(false); // 2. Tutup sidebar setiap kali pindah halaman
  }, [location]);
  ...
  const activeEmployees = employees.filter(e => e.status_kerja?.nama_status === 'tetap' || e.status_kerja?.nama_status === 'kontrak').length;
  const DEPARTMENTS_CONFIG = useMemo(() => {
    const counts = {};
    for (const emp of employees) {
      const key = emp.jabatan?.nama_jabatan; 
      if (key && DEPARTMENTS_CONFIG_RAW[key]) { 
        counts[key] = (counts[key] || 0) + 1;
      }
    }
    const config = {};
    for (const deptName in DEPARTMENTS_CONFIG_RAW) {
      config[deptName] = {
        ...DEPARTMENTS_CONFIG_RAW[deptName],
        count: counts[deptName] || 0
      };
    }
    return config;
  }, [employees]);
